...
#define BINARY_OP(name, opType, symbol)                                     \
  ...
  inline Expression name(lhs_t lhs, rhs_t rhs) requires(                    \
      std::derived_from<lhs_t, Value> || std::derived_from<rhs_t, Value>) { \
...
#define BINARY_OP_AND_SYMBOL(name, opType, symbol)                          \
  ...
  template <typename lhs_t, typename rhs_t>                                 \
  inline Expression operator symbol(lhs_t lhs, rhs_t rhs) requires(         \
      std::derived_from<lhs_t, Value> || std::derived_from<rhs_t, Value>) { \
    return name(lhs, rhs);                                                  \
  }
